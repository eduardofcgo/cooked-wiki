...
function ReportCookedModal({ visible, onClose, cookedId }) {
  ...
  const onConfirmReport = async () => {
    ...
    try {
      // Report the cooked content with the provided reason
      await profileStore.reportCooked(credentials.username, cookedId, reportReason)

      showInAppNotification(ActionToast, {
        props: { message: 'Cooked content has been reported' },
      })
    } catch (error) {
      showInAppNotification(ActionToast, {
