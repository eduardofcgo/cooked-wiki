...
import { View, SafeAreaView, StyleSheet, Text, DeviceEventEmitter, Platform, StatusBar, Alert } from 'react-native'
import { useRoute, usePreventRemove, useNavigationState, useFocusEffect } from '@react-navigation/native'
...
function EditRecipeText({ navigation }) {
  ...
  const handlePreviewRecipe = useCallback(() => {
    if (Platform.OS === 'android') {
      Alert.alert('Preview Not Available', 'Preview mode is not supported on Android yet.', [{ text: 'OK' }])
      return
    }
  ...
  const refreshRecipeScreensInStack = useCallback(() => {
    // TODO: moves into a store like RecipeStore
